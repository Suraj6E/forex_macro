"""Seed the source register, instruments and known dislocations.

planning.md §5 (source register), locked decision 6 (majors only) and §4.6
(what a 2007 start actually contains).

Idempotent: re-running updates the descriptive fields and leaves health,
history and any hand-edited mapping alone.
"""
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        for spec in SOURCES:
            ...
            self.stdout.write(f"{'created' if created else 'updated'} source {source.key}")
        ...
        self.stdout.write(f"{len(MAJORS)} instruments present")

        for spec in RELEASE_GROUPS:
            ReleaseGroup.objects.update_or_create(
                key=spec["key"], defaults={k: v for k, v in spec.items() if k != "key"}
            )
        self.stdout.write(f"{len(RELEASE_GROUPS)} release groups present")
        ...
        self.stdout.write(f"{len(MARKET_EVENTS)} market events present")

        self.stdout.write(
            self.style.SUCCESS(
                "Reference data seeded. Only forexfactory_weekly has a collector so "
                "far; the rest are register rows waiting for one."
            )
        )
